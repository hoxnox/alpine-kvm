...
sudo ./alpine-make-vm-image --keys-dir keys --image-format qcow2 --image-size 100G --repositories-file example/repositories --packages "$(cat example/packages)" --script-chroot alpine-virthardened-$(date +%Y-%m-%d).qcow2 -- ./example/configure.sh
